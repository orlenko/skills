...
LAUNCHER = ROOT / "bin" / "agent-orchestra"


class LauncherTests(unittest.TestCase):
    def setUp(self) -> None:
        temp = tempfile.TemporaryDirectory(prefix="agent-orchestra-launcher-")
    ...
    def run_launcher(self, path: str, **env: str) -> subprocess.CompletedProcess:
        environ = {key: value for key, value in os.environ.items() if key != "AGENT_ORCHESTRA_PYTHON"}
    ...
    def test_a_version_manager_shim_first_on_path_is_skipped(self) -> None:
        ...
        self.assertTrue(result.stdout.startswith("agent-orchestra "))
    ...
    def test_the_environment_override_wins(self) -> None:
        result = self.run_launcher(str(self.shims), AGENT_ORCHESTRA_PYTHON=sys.executable)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertFalse(self.marker.exists())
